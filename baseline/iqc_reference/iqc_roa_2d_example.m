% ============================================================================
%  IQC BASELINE  -  Theorem 1 of  H. Yin, P. Seiler, M. Arcak,
%  "Stability Analysis Using Quadratic Constraints for Systems with Neural
%   Network Controllers," IEEE Trans. Automatic Control, 67(4):1980-1987, 2022.
%
%  NOTE ON THE NAME: despite the file name, the plant encoded below is the
%  2-D LTI example of THIS paper.  The discrete-time matrices are a sampling
%  (dt = 0.02) of  A = [-5 1; 3 -5],  B = [0.5; 1]:
%      AG = [-5*dt+1, dt; 3*dt, -5*dt+1],   BG2 = [0.5*dt; dt].
%  This is the script behind the "IQC method [3]" row of Table I (10/10/1).
%
%  Derived from the MIT-licensed repository
%      https://github.com/heyinUCB/IQCbased_ImitationLearning
%      Copyright (c) 2021 heyinUCB   (full text in ./UPSTREAM_LICENSE)
%  Requires MATLAB + CVX + an SDP solver (e.g. MOSEK).
% ============================================================================

% Region of attraction analysis on the pendulum system with NN controller
clear
clc

%% parameters
g = 10; % gravitational coefficient
m = 0.15; % mass
l = 0.5; % length
mu = 0.05; % frictional coefficient
dt = 0.02; % sampling period

%% x^+ = AG*x + BG1*q + BG2*u
AG = [-5*dt+1,      dt;...
      3*dt, -5*dt+1];
% describes how q enters the system
% BG1 = [0;...
%        -g/l*dt];
% describes how u enters the system
BG2 = [0.5*dt;...
      dt];
nG = size(AG, 1);
nu = 1;
% nq = 1;

%  v_Delta = CG*xG + [DG1 DG2]*[q; u] = xG
CG = eye(2);
DG1 = 0;
DG2 = 0;

%% load weights and biases of the NN controller     
%fname = '../vehicle_training/Wb_s32_tanh/';
fname = 'Wb_s32_tanh/';
W1 = load([fname 'W1final.csv']);
W2 = load([fname 'W2final.csv']);
W3 = load([fname 'W3final.csv']);
% W1=W1';
% W2=W2';
% W3=W3';
n1 = size(W1,1);
n2 = size(W2,1);
n3 = size(W3,1);
nphi = n1+n2;

b1 = zeros(n1,1);
b2 = zeros(n2,1);
b3 = zeros(n3,1);

%% bounds for the inputs to the nonlinearity
tic
xeq = [0.0; 0.0];
v1eq = W1*xeq + b1;
w1eq = tanh(v1eq);
v2eq = W2*w1eq + b2;
w2eq = tanh(v2eq);
v3eq = W3*w2eq + b3; % This is also u_*
% usat = sat(v3) = sat(u)

deltav1 = 0.5;
v1up = deltav1 + v1eq;
v1lb = v1eq - deltav1;

% 
alpha1 = min((tanh(v1up)-tanh(v1eq))./(v1up-v1eq), (tanh(v1eq)-tanh(v1lb))./(v1eq-v1lb));
beta = 1;
w1up = tanh(v1up);
w1lb = tanh(v1lb);

v2up = [];
v2lb = [];
for i = 1:n2
    v2up_i = W2(i,:)*1/2*(w1up+w1lb) + b2(i) + abs(W2(i,:))*1/2*abs(w1up-w1lb);
    v2lb_i = W2(i,:)*1/2*(w1up+w1lb) + b2(i) - abs(W2(i,:))*1/2*abs(w1up-w1lb);
    v2up = [v2up; v2up_i];
    v2lb = [v2lb; v2lb_i];
end
alpha2 = min((tanh(v2up)-tanh(v2eq))./(v2up-v2eq), (tanh(v2eq)-tanh(v2lb))./(v2eq-v2lb));

Alpha = blkdiag(diag(alpha1),diag(alpha2));
Beta = beta*eye(nphi);

% Filter for sector IQC
Psi_phi = [Beta, -eye(nphi);...
          -Alpha, eye(nphi)];

%% Define IQCs for the nonlinearity x1 - sin(x1)
% x1 - sin(x1) is slope-restricted in [0, 2] globally ---> off-by-one IQC

% x1 - sin(x1) is sector-bounded in [0, 1.22] globally ---> sector IQC
% x1 - sin(x1) is sector-bounded in [0, 1] locally on x1 in [-pi,pi]
% x1 - sin(x1) is sector-bounded in [0, 0.7606] locally on x1 in [-2.5,2.5]
% define the filter for off-by-one IQC
% L_slope = 2;
% m_slope = 0;
% Apsi = 0;
% nxi = size(Apsi,1);
% Apsi = Apsi*dt + eye(nxi);
% Bpsi1 = -L_slope;
% Bpsi2 = 1;
% Bpsi1 = Bpsi1*dt;
% Bpsi2 = Bpsi2*dt;
% Cpsi = [1; 0];
% Dpsi1 = [L_slope; -m_slope];
% Dpsi2 = [-1; 1];
% % M matrix for off-by-one IQC      
% M_off = [0, 1;...
%          1, 0];
% 
% % define the filter for sector IQC
% L_sector = 0.7606;
% m_sector = 0;
% Psi_sec = [L_sector, -1;...
%            -m_sector,    1];
% % M matrix for sector IQC
% M_sec = [0, 1;...
%          1, 0];
% 
%%
N = blkdiag(W1,W2,W3);
Nux = N(n1+n2+1:end, 1:nG);
Nuw = N(n1+n2+1:end,nG+1:end);
Nvx = N(1:n1+n2, 1:nG);
Nvw = N(1:n1+n2,nG+1:end);
Nuzeta = Nux;% [Nux, zeros(nu,nxi)];
Nvzeta = Nvx;%[Nvx, zeros(nphi,nxi)];

%% construct the extended system
Abar = AG% [AG, zeros(nG,nxi);...
        %Bpsi1*CG, Apsi];
Bbar = BG2%[BG1, BG2;...
        %Bpsi1*DG1+Bpsi2, Bpsi1*DG2];
Cbar = CG% [Dpsi1*CG, Cpsi];
Dbar = 0;%[Dpsi1*DG1+Dpsi2, Dpsi1*DG2];
nzeta = nG;

%% Convex Optimization - compute ROA
cvx_begin sdp quiet
    % cvx_solver mosek
    
    % Variables
    variable P(nzeta,nzeta) symmetric;
    variable Tphi(nphi,nphi) diagonal; 
    variable lambda1;
    variable lambda2;
    lambda1 >= 0;
    lambda2 >= 0;
    Tphi(:) >= 0;
    P >= 1e-8*eye(nzeta);

    % Term to bound nonlinearity Phi
    Mphi = [zeros(nphi), Tphi; Tphi, zeros(nphi)];
%     Rphi = [W1, zeros(n1, nxi+nphi+nq);...
%             zeros(n2, nx), W2, zeros(n2,n2+nusat+nq);...
%             zeros(n3, nx+n1),W3,zeros(n3,nusat+nq);...
%             zeros(nphi,nx),eye(nphi), zeros(nphi, nq)];     
    Rphi = [Nvzeta, Nvw;...
            zeros(nphi,nzeta),eye(nphi)];
    Qphi = Rphi'*Psi_phi'*Mphi*Psi_phi*Rphi;
    
    % Term to represent V(k+1) - V(k)
    S = [Abar'*P*Abar - P, Abar'*P*Bbar;...
         Bbar'*P*Abar, Bbar'*P*Bbar];
%     RV = [eye(nx), zeros(nx,nphi + nq);...
%           zeros(nq, nx + nphi), eye(nq);...
%           zeros(nu, nx + n1 + n2), eye(nu), zeros(nu, nq)];
    RV = [eye(nzeta), zeros(nzeta,nphi);...
          Nuzeta, Nuw];
    QV = RV'*S*RV;
    
    % Term that uses off-by-one IQC
    % Qoff = lambda2*RV'*[Cbar Dbar]'*M_off*[Cbar Dbar]*RV;
    
    % % Term that uses sector IQC
    % R_sec = [eye(1,1),zeros(1,1+nxi+nphi+nq);...
    %          zeros(nq,nzeta+nphi),eye(nq)];
    % Qsec = lambda1*R_sec'*Psi_sec'*M_sec*Psi_sec*R_sec;
    
    % Matrix Inequality
    QV + Qphi <= -1e-10*eye(nzeta+nphi);
    for i = 1:n1
    % enforce {x: x'Px<=1} \subset {x: |[W0(i,:) 0]*x| <= r1} 
        [deltav1^2, W1(i,:);...
         W1(i,:)', P] >=0;...
    end
    % enforce {x: x'Px<=1} \subset {x: |x1| <= x1bound}
    % x1bound = 2.5;
    % [x1bound^2, [1,0, zeros(1,nxi)];...
    % [1,0, zeros(1,nxi)]', P] >=0;...
        x1bound = 2.5;
    [x1bound^2, [1,0];...
     [1,0]', P] >=0;...
    minimize(trace(P(1:nG,1:nG)))
cvx_end
toc
P
if ~isnan(P)
    radii = 1./sqrt(eig(P(1:nG,1:nG)))
end

%% plot results
% Simulation results
N1 = 20;
x1box = linspace(-5,5,N1);
N2 = 20;
x2box = linspace(-9,9,N2);
xIC = [x1box x1box  x1box(1)*ones(1,N1) x1box(end)*ones(1,N1); ...
    x2box(1)*ones(1,N2) x2box(end)*ones(1,N2) x2box x2box];
Nstep = 50;
for i=1:size(xIC,2)
    x0 = xIC(:,i);
    [x,u] = nnclosedloop(Nstep,x0,{W1,W2,W3},{b1,b2,b3},g,m,l,mu,dt);
    plot(x(1,:),x(2,:),'--','color',mycolor('lightgray'));
    hold on
end

%% Hyperplanes 
x1 = linspace(-10,10,1e3);
figure(1)
for i=1:n1
    plot(x1,(deltav1-W1(i,1)*x1)/W1(i,2),'color',mycolor('orange')) 
    plot(x1,(-deltav1-W1(i,1)*x1)/W1(i,2),'color',mycolor('orange'))
    hold on;
end
plot([x1bound,x1bound],[-9,9],'color',mycolor('maroon'))
plot([-x1bound,-x1bound],[-9,9],'color',mycolor('maroon'))

% ROA
% pvar x1 x2
% V = [x1,x2]*P(1:nG,1:nG)*[x1;x2];
% domain1 = [-10, 10, -10, 10];
% [C,h] = pcontour(V,1,domain1,'r');
% h.LineColor = mycolor('coolblue');
% h.LineWidth = 4;
% hold on
% 
% % xeq
% plot(xeq(1),xeq(2),'kx','MarkerSize',10);
% hold off;
% 
% % ah = [];
% % ah(1)=annotation('textarrow',[0.58 0.65],[0.85 0.65],'String','$\{x:\underline{v}^1 \leq v^1 \leq \bar{v}^1\}$','interpreter','latex');
% % ah(2)=annotation('textarrow',[0.45 0.33],[0.18 0.35],'String','$\{x:\underline{\theta} \leq \theta \leq \bar{\theta}\}$','interpreter','latex');
% % set(ah,'FontSize',18)
% 
% grid on;
% axis([-5 5 -9 9]);
% xlabel('$x_1$','interpreter','latex')
% ylabel('$x_2$','interpreter','latex')
% 
% garyfyFigure